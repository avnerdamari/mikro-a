...
const MCQ: McqQuestion[] = [
  ...
  { id: 'el-q4', question: 'מוצר עם E=0 (קשיח מושלם, כמו אינסולין לחולה סוכרת) — מה קורה לכמות הנדרשת כשהמחיר עולה משמעותית?', options: ['הכמות יורדת בהתאם', 'הכמות לא משתנה בכלל', 'הכמות עולה', 'אי אפשר לדעת'], correct: 1, explanation: 'E=0 אומר שהכמות המבוקשת אינה מגיבה כלל לשינוי במחיר — הצרכן חייב לרכוש אותה כמות בכל מחיר (למשל תרופה חיונית).' },
  { id: 'el-q5', question: 'ביקוש קשיח (E<1). המחיר יורד. מה קורה להכנסה הכוללת (TR)?', options: ['TR עולה', 'TR יורד', 'TR לא משתנה', 'תלוי בכמות ההתחלתית'], correct: 1, explanation: 'בביקוש קשיח, שינוי באחוזים בכמות קטן משינוי באחוזים במחיר. ירידת מחיר תגרום לעלייה קטנה בכמות שלא תפצה על אובדן ההכנסה למחיר — TR יורד.' },
  { id: 'el-q6', question: 'גמישות צולבת בין שני מוצרים היא שלילית (E_c<0). מה זה אומר על הקשר ביניהם?', options: ['הם תחליפים', 'הם מוצרים משלימים — עלייה במחיר אחד מפחיתה את הביקוש לשני', 'אין קשר ביניהם', 'שניהם מוצרים נחותים'], correct: 1, explanation: 'גמישות צולבת שלילית: עלייה במחיר מוצר Y מפחיתה את הביקוש למוצר X — סימן שהם נצרכים יחד (משלימים), כמו מכונית ודלק.' },
  { id: 'el-q7', question: 'למה חשוב לספק/למוכר לדעת אם הביקוש למוצר שלו גמיש או קשיח, לפני שמעלים מחיר?', options: ['זה לא ממש משנה לו', 'כי זה קובע אם העלאת המחיר תגדיל או תקטין את ההכנסה הכוללת שלו', 'זה משפיע רק על עלות הייצור', 'גמישות קובעת רק את המיסוי'], correct: 1, explanation: 'אם הביקוש גמיש, העלאת מחיר תקטין TR (לא כדאי); אם קשיח, העלאת מחיר תגדיל TR (כדאי). זו בדיוק ההחלטה העסקית שגמישות המחיר עוזרת לקבל.' },
  { id: 'el-q8', question: 'מוצר עם תחליפים רבים וזמינים (למשל מותג ספציפי של קורנפלקס) צפוי להיות בעל גמישות מחיר:', options: ['גבוהה (גמיש) — קל לצרכן לעבור למוצר דומה אם המחיר עולה', 'נמוכה (קשיח) — אין לצרכן ברירה', 'אפס בדיוק', 'שלילית'], correct: 0, explanation: 'ככל שיש יותר תחליפים זמינים וקרובים, קל יותר לצרכן לעבור אליהם כשהמחיר עולה — ולכן הביקוש למוצר הספציפי גמיש יותר.' },
]
